...
def test(model_to_test, current_fold, plane, rst_dir, vis):
    ...
    for i in range(total):
        ...
        case_num = s[1].split("pancreas_")[1].split(".")[0]
        ...
        for sli in range(label_.shape[0]):
            try:
                ...
                out_ori = (model.predict(image_padded_prep) > 0).astype(np.uint8)#
        ...
        cur_dsc = DSC_computation(label_, pred)
        #cur_dsc = Jaccard(pred, label_)
        print(cur_dsc)
